# Serializer class for our users
...
class LoginSerializer(serializers.Serializer):
    """
    Serializes login info to log into the system
    """
    ...
    def validate(self, data):
        ...
        # Raise an exception if email isn't provided
        if email is None:
            raise serializers.ValidationError(
                'An email address is required to log in.'
            )

        # Raise an exceptino if a password is not provided
        if password is None:
            raise serializers.ValidationError(
        ...
        # we pass email as the username value since in our User mode we
        # set USERNAME_FIELD as email
        ...
        # Django provies a flag on our User model called is_active. The
        # purpose of this flag is to tell us whether the user has been banned
        # or deactivated. This will almost never be the case, but it is worth
        # checking. Raise an exception if it is deactivated
        if not user.is_active:
            raise serializers.ValidationError(
                'This user has been deactivated.'
            )

        # the validate method shoudl return a dictionary of validated data.
        # This is the data that is passed to the create and update methods that
        # we'll see later on
        return {
            'email': user.email,
